...
class RestclientController extends Controller implements Iterator, ArrayAccess {
    ...
    public function apiAction($get, $post){
        //alterar a url abaixo; apontar para o endereço de onde esta a API
        $api_url = 'http://devphpsolucoes.com.br/teste-rest/server-rest/';
        $api_data = ['cnpj' => $post['cnpj']];
        // 31804115000243

        //$api = new RestClient;
        $result = $this->post($api_url . "buscar", $api_data);
        $response_json = $result->decode_response();

        $sintegra = new RestclientModel();
        $sintegra->set("idusuario", $_SESSION["usuario"][0]["id"]);
        $sintegra->set("cnpj", $post['cnpj']);
        $sintegra->set("resultado_json", json_encode($response_json));
        $sintegra->salvar();
