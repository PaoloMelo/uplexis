...
class ValidaCPFCNPJ
{
	...
	public function retorna_cpf_cnpj () 
	{
	...
	//PREPARANDO PRA REMOVER ACENTOS NO WHILE 
	function utf8_strtr($str) {
		$from = "áàãâéêíóôõúüçÁÀÃÂÉÊÍÓÔÕÚÜÇćč";
		$to = "aaaaeeiooouucAAAAEEIOOOUUCcc";
		$keys = array();
		$values = array();
	...
	function __autoload($v){
	    // coloquei os caminhos onde vamos procurar a classe quando ela for chamada
	    $paths = array(
	        SISTEMA . $v . '.php', // procurar dentro da pasta de sistema
	        SISTEMA . 'helpers/' . $v . '.php', // procurar dentro da pasta de sistema
	        CONTROLLERS . $v . '.php', // procurar dentro da controllers
	        MODELS . $v . '.php', // procurar dentro da models
	    );
//printa($paths);
	    // ai eu percorro essas pastas e vejo se o arquivo existe
	    foreach ($paths as $p) {
